...
import com.nikita23830.animearts.client.ClientProxy;
...
public class AnimeRenderDefault extends TileEntitySpecialRenderer {

    public AnimeRenderDefault() {
    ...
    public void renderTileEntityAt(TileEntity tileEntity, double x, double y, double z, float f) {
        ...
        DefaultAnimeTile tile = (DefaultAnimeTile)tileEntity;
        ...
        if (!(block instanceof DefaultArts))
            return;
        this.draw(tileEntity,(float)x,(float)y,(float)z, Integer.toString(mt + 1), 2.5F, tile.getIdArt());
    }

    private void draw(TileEntity tileEntity, float x,float y, float z, String corona, float offset, String mt) {
        if (mt.isEmpty())
            return;
        ...
        float g = RenderManager.instance.playerViewY;
        ...
        draw(corona, mt, -64/2, -56, 64.0D, 64.0D, 1.0F);
        GL11.glDisable(3042);
        GL11.glEnable(2896);
    ...
    public static void draw(String img, String mt, double x, double y, double w, double h, float alpha) {
        int n = NumberUtils.isNumber(img) ? NumberUtils.toInt(img) : 0;
        n = Math.max(Math.min(n, 15), 0);
        if (!mt.isEmpty()) {
            if (CommonProxy.register.arts.get(mt).needDownload && !CommonProxy.register.arts.get(mt).thread.finalize)
                return;
            if (CommonProxy.register.arts.get(mt).texture == null)
                return;
            GL11.glBindTexture(GL11.GL_TEXTURE_2D, CommonProxy.
                    register.
                    arts.
                    get(mt).
                    texture.
